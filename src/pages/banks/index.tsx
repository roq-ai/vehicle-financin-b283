...
import { useRoqClient, useBankFindManyWithCount } from 'lib/roq';
...
export function BankListPage(props: BankListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['user', 'loan_officer.count', 'vehicle_loan.count'],
  ...
  const { data, error, isLoading, mutate } = useBankFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.bank.delete({ where: { id } });
      await mutate();
    } catch (error) {
  ...
  const columns: ColumnType[] = [
    ...
    hasAccess('loan_officer', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ...
      : null,
    hasAccess('vehicle_loan', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)
      ? {
          id: 'vehicle_loan',
          header: 'Vehicle Loan',
          accessorKey: 'vehicle_loan',
          cell: ({ row: { original: record } }: any) => record?._count?.vehicle_loan,
        }
      : null,
