...
import { useRoqClient, useVehicleLoanFindFirst } from 'lib/roq';
...
function VehicleLoanEditPage() {
  ...
  const { data, error, isLoading, mutate } = useVehicleLoanFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: VehicleLoanInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.vehicle_loan.update({
        data: values as RoqTypes.vehicle_loan,
        where: {
          id,
        },
      });
