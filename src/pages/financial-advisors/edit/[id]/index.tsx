...
import { useRoqClient, useFinancialAdvisorFindFirst } from 'lib/roq';
...
function FinancialAdvisorEditPage() {
  ...
  const { data, error, isLoading, mutate } = useFinancialAdvisorFindFirst(queryParams, {}, { disabled: !id });
  const [formError, setFormError] = useState(null);

  const handleSubmit = async (values: FinancialAdvisorInterface, { resetForm }: FormikHelpers<any>) => {
    setFormError(null);
    try {
      const updated = await roqClient.financial_advisor.update({
        data: values as RoqTypes.financial_advisor,
        where: {
          id,
        },
      });
